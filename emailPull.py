...
def ReadEmail():
    ...
    # it will return with its status and a list of ids
    # status, data = mail.search(None, '(FROM Jamie SINCE 01-DEC-2020)')
    # status, data = mail.search(None, '(FROM Jamie SINCE {})' .format(TIME))
    status, data = mail.search(None, 'FROM Jamie') #use for testing
    #status, data = mail.search(None, 'FROM Jamie UNSEEN') #use for final product
